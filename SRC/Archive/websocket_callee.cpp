...
void add(autobahn::wamp_invocation invocation)
{
    ...
    std::cerr << "Procedure com.example.add invoked: " << a << ", " << b << std::endl;
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        auto transport = std::make_shared < autobahn::wamp_websocketpp_websocket_transport<websocketpp::config::asio_client> >(
          ws_clinet, "ws://192.168.1.101:8080/ws", debug);
        ...
        connect_future = transport->connect().then([&](boost::future<void> connected) {
            ...
            start_future = session->start().then([&](boost::future<void> started) {
                ...
                join_future = session->join(parameters->realm()).then([&](boost::future<uint64_t> joined) {
                    ...
                    provide_future = session->provide("com.example.add", &add).then(
