...
IUIAutomation * g_pSetAutomation = NULL;


HRESULT InitializeSetUIAutomation()
{
    CoInitialize(NULL);
    HRESULT hr =
...
// Initialize the UIA object
    ...
    HRESULT hr  = InitializeSetUIAutomation();
    if (FAILED(hr)) {
        std::cerr << "InitializeSetUIAutomation failed.  Exiting..." << std::endl;
        exit(0);
    }

// hop to the node with the value one wishes to change...
    
    pFound = hopToNode( g_pSetAutomation, invocation, input_array );
    ...
    for ( unsigned i = 0; i < INPUT_ARRAY_MAX; )
    {
        ...
        if ( inst == multibyte2widechar ( "set" )) {
            std::cerr << "Found set instruction, breaking out of loop" << std::endl;
            break;
        }
    }
    ...
    if ( inst != multibyte2widechar ( "set" ))
    {
        std::cerr << "DEBUG: could not find a \"set\" instruction in argument array. Exiting..." << std::endl;
	    exit(0);
    }
